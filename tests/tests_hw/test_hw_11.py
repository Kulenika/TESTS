...
def test_web_tables(browser):
    ...
    web_tables.add_button.click() #открывается окно и сразу же закрывается, тест падает
    time.sleep(3)
    assert web_tables.modal_window.alert()
    web_tables.submit_button.click()
    web_tables.modal_window.alert()
    ...
    time.sleep(3)
    web_tables.submit_button.click()
    assert not web_tables.modal_window.alert()
    #как написать проверку, что новая запись добавлена в таблицу?
    web_tables.edit_4_button.click()
    assert web_tables.modal_window.alert()
    assert web_tables.first_name_mw.get_text() == 'Anna'
    assert web_tables.last_name_mw.get_text() == 'Kulevova'
    assert web_tables.e_mail_mw.get_text() == 'zizizi@mail.ru'
    assert web_tables.age_mw.get_text() == '22'
    assert web_tables.salary_mw.get_text() == '5000'
    assert web_tables.department_mw.get_text() == 'Legal'
